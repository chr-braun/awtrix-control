    ...
    action:
      - service: awtrix_control.send_sensor
        data:
          ...
          sensor_entity: sensor.temperatur_wohnzimmer
          slot: 0
          color: "#FF6B35"
      - delay: "00:00:02"
      - service: awtrix_control.send_sensor
        data:
          ...
          sensor_entity: sensor.luftfeuchtigkeit_wohnzimmer
          slot: 1
          color: "#00A3FF"
      - delay: "00:00:02"
      - service: awtrix_control.send_sensor
        data:
          sensor_name: "Druck"
          sensor_entity: sensor.luftdruck
          slot: 2
          color: "#9C27B0"

  # Bei Sensor-Änderungen (optional - kann zu vielen Nachrichten führen)
  - alias: "AWTRIX Temperatur Update"
    description: "Sendet Temperatur-Änderungen an AWTRIX"
    trigger:
      - platform: state
        entity_id: sensor.temperatur_wohnzimmer
        for: "00:01:00"  # Nur bei Änderungen länger als 1 Minute
    action:
      - service: awtrix_control.send_sensor
        data:
          sensor_name: "Temperatur"
...
`;
        ...
        this.selectedSensors.forEach((sensor, index) => {
            ...
            sensorCode += `      - service: awtrix_control.send_sensor
        data:
          sensor_name: "${sensor.name}"
          sensor_entity: ${sensor.id}
          slot: ${slot}
...
`;
            if (index < this.selectedSensors.length - 1) {
                sensorCode += `      - delay: "00:00:02"
`;
            }
        });

        return code.replace(/# Ersetze Platzhalter.*$/s, sensorCode);
    }
